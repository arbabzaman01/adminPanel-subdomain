        ...
        "Dashboard.useEffect": ()=>{
            if ("TURBOPACK compile-time truthy", 1) {
                ...
                if (isAuthenticated !== "true" || !role) {
                    router.replace("/"); // use replace to avoid back button going to dashboard
                } else {
                    setUserRole(role);
                }
                setIsCheckingAuth(false); // complete auth check
            }
        }
    }["Dashboard.useEffect"], [
        router
...
_s(Dashboard, "oy4rbmEhGHkQqG4XWjRQzFH9Jzs=", false, function() {
    return [
